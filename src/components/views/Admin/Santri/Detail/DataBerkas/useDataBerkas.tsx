...
const useDataBerkas = () => {
  ...
  const getFiles = async () => {
    const res = await fileService.getBySantriId(id);
  ...
  const updateFiles = async ({ id, payload }: { id: number; payload: FileSchemaType }) => {
    if (!dataFiles) {
      const { data } = await fileService.create(id, payload);
      return data.data;
    } else {
      const { data } = await fileService.update(id, payload);
      return data.data;
    }
  };
