...
const useDataAlamat = () => {
  ...
  const updateAddress = async ({ id, payload }: { id: number; payload: AddressSchemaType }) => {
    if (!dataAddress) {
      const { data } = await addressService.create(id, payload);
      return data.data;
    } else {
  ...
  const handleUpdateAddress = (data: AddressSchemaType) => {
    ...
    mutateUpdateAddress({ id, payload });
  };
