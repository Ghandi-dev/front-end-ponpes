...
const CetakFormulir = () => {
  ...
  // Auto print setelah semua data tersedia
  useEffect(() => {
    if (dataAddress && dataVillage && isSuccessDataVillage) {
      setTimeout(() => {
        window.print();
      }, 500);
    }
  }, [dataAddress, isSuccessDataVillage]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4 m-0 print:bg-white">
      {dataProfile?.santri?.status == SANTRI_STATUS.RE_REGISTERED ? (
        ...
        <div className="w-[210mm] h-[297mm] bg-white shadow-lg p-10 relative border print:shadow-none print:border-none">
          <div className="flex justify-center items-center h-full">
            <h1 className="text-2xl font-bold text-center">
              Formulir Pendaftaran Belum Tersedia, Pastikan Langkah Sebelumnya Sudah Dijalankan Terlebih Dahulu
            </h1>
          </div>
        </div>
      )}
